...
int main(int argc, char **argv)
{
  ...
  for(int j = 0; j < 4; j++) {
    ...
    // printf("Doing CPU and GPU Vector add..\n");
    for(int k = 0; k < 100; k++ ) {       // calculating the time for 100 iterations to get the average
      ...
      time_d = time_d + (double)(end_d-start_d)/CLOCKS_PER_SEC;
      time_h = time_h + (double)(end_h-start_h)/CLOCKS_PER_SEC;
      cudaMemcpy(c, c_d, nBytes, cudaMemcpyDeviceToHost);
    }

    printf("Number of elements: %d, GPU Time: %f, CPU Time: %f\n", n, time_d / 100, time_h / 100);
    cudaFree(a_d);
    cudaFree(b_d);
    cudaFree(c_d);
  }
